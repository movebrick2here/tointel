...
business.LEVEL = {}
business.LEVEL.ONE = 300
business.LEVEL.TWO = 400
business.LEVEL.THREE = 600
business.LEVEL.FOUR = 800
business.LEVEL.FIVE = 1000
...
function business:results_string_to_number(info)
    ...
    for i = 1, num do
        ...
        info.list[i].threshold_consumption = business.LEVEL.THREE
    end
end
...
function business:make_conditions(tbl)
    ...
    conditions.item_tbl.hour_consumption = tostring(business.LEVEL.THREE) .. " EGT"
...
function business:do_action(tbl)
    -- 封装查询条件和排序字段以及分页
    local columns = { "terminal_id", "terminal_name", "device_id", "terminal_sn", "terminal_version", "running_time", "hour_consumption",  "create_time", "update_time" }
