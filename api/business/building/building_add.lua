...
function business:add_timestamp(tbl)
    ...
    tbl.building_id = "B".. string.upper(uuid())
end
...
function business:do_action(tbl)
    ...
    local result,errmsg = check:name_is_exists(tbl.building_name)
    if true == result then
        return false, "数据库中已有名称:".. tbl.building_name .. "的记录"
    end
