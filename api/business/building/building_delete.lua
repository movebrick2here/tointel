...
function business:array_to_string(ids)
    ...
    for i = 1, num do
        ...
        parent_like_conditions = parent_like_conditions .. "building_f_tree like '%" .. tostring(ids[i]) .. "%'"
        if i ~= num then
            id_list = id_list .. ","
...
function business:query_children_list(ids)
    ...
    -- 查询记录
    local sql = "select building_id from t_building where building_id in (" .. ids_list .. ") or " .. parent_like_conditions
    local configure = require "configure"
    local dao = require "dao"
    local LOG = require "log"
    LOG:DEBUG("query table:t_building" .. " sql:" .. sql)
    local result,info = dao:query_by_sql(configure.DBCService, sql)

    local cjson = require "cjson"
    if false == result then
        LOG:ERROR("query table:t_building " .. " value:" .. cjson.encode(tbl) .. " failed msg:" .. info)
        return false,info
    end

    LOG:DEBUG("query table:t_building " .. " value:" .. cjson.encode(tbl) .. " response:" .. cjson.encode(info))
    ...
    for i = 1, #info.list do
        table.insert(id_array, info.list[i].building_id)
    end    
...
function business:do_action(ids)
    ...
    local check = require "building_check"
    local result = check:check_building_has_device(children_list_str)
    if true == result then
        return false, "建筑物已经有设备关联，无法删除"
    end
    ...
    -- 删除记录
    local sql = "delete from t_building where building_id in (" .. ids_list .. ") or " .. parent_like_conditions
    local configure = require "configure"
    local dao = require "dao"
    local LOG = require "log"
    LOG:DEBUG("delete table:t_building" .. " sql:" .. sql)
