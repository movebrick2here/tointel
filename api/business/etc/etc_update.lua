...
function business:make_conditions(tbl)
    ...
    conditions.item_tbl.etc_id = tbl.etc_id
    return conditions
end
...
function business:do_action(tbl)
    if nil ~= tbl.etc_name then
        -- 检查名称是否重复
        local check = require "etc_check"
        local result,errmsg = check:name_is_exists(tbl.etc_name)
        if true == result then
            return false, "数据库中已有名称:".. tbl.etc_name .. "的记录"
        end
