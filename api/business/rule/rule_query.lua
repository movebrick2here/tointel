...
function business:make_conditions(id)
    ...
    conditions.item_tbl.rule_id = id
    return conditions
end
...
function business:do_action(id)
    ...
    -- 查询记录
    local columns = { "rule_id", "rule_name", "rule_content", "description",  "create_time", "update_time" }
    ...
    local cjson = require "cjson"
    LOG:DEBUG("query table:" .. table_name .. " rule_id:" .. id)
    local result,info = dao:query(configure.DBCService, table_name, columns, conditions)
    if false == result then
        LOG:ERROR("query table:" .. table_name .. " rule_id:" .. id .. " failed msg:" .. info)
        return false,info
    end
