...
function business:do_action(id)
    ...
    -- 查询记录
    local columns = { "dept_id", "dept_name", "parent_id", "description",  "create_time", "update_time" }
    ...
    local cjson = require "cjson"
    LOG:DEBUG("query table:" .. table_name .. " dept_id:" .. id)
    local result,info = dao:query(configure.DBCService, table_name, columns, conditions)
    if false == result then
        LOG:ERROR("query table:" .. table_name .. " dept_id:" .. id .. " failed msg:" .. info)
        return false,info
    end
