...
function business:results_string_to_number(info)
    ...
    for i = 1, num do
        ...
        if ( nil ~= info.list[i].date) then
            info.list[i]["date"] = tonumber(info.list[i].date)
        end
...
function business:do_action(tbl)
    -- 封装查询条件和排序字段以及分页
    local columns = { "terminal_id", "terminal_name", "device_id", "terminal_sn", "terminal_version", "stat as detail", "running_time", "hour_consumption", "consumption", "total_consumption", "stat_time as date", "create_time", "update_time" }
