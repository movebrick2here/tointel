...
if OP == "add" then
    ...
    if true == result then
        local business = require "user_add"
        ...
        if false == result then
            ...
            response.data = {}
            response.data.user_name = tbl.user_name
...
elseif OP == "update" then
    ...
    if true == result then
        local business = require "user_update"
...
elseif OP == "query" then
    ...
    if true == result then
        local business = require "user_query"
        local result,info = business:do_action(tbl.user_id)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
        else
...
elseif OP == "delete" then
    ...
    if true == result then
        local business = require "user_delete"
        local result,errmsg = business:do_action(tbl.user_id)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = errmsg
        end
...
elseif OP == "query_list" then
    ...
    if true == result then
        local business = require "user_query_list"
