...
--     content_by_lua_file ${HOME_DIR}/interface/dept.lua;
-- }
-- 浏览器请求的地址是 http://${DOMAIN}/interface/dept/${OP}
-- DOMAIN 服务器部署的域名
-- OP的值取决于代码中的实现
...
-- RESTFUL API定义格式见 https://github.com/changyuezhou/api_base/blob/master/doc/dept.md
-- *********************************************************************************************************
...
function check_delete_params(tbl)
    ...
    if (nil == tbl["dept_id"]) or
            ("table" ~= type(tbl["dept_id"])) then
        return false, "请检查参数dept_id.为必填且必须为字符数组"
    end
...
-- 返回值:
-- result: bool 表示函数成功或者失败
-- dept: table 用户信息对象(包含用户ID, 组ID等),失败时为nil
-- redirect_url: string 用户登录页面URL
-- #########################################################################################################
function get_dept_info()
    local auth_api = require "authorization"
    local result,dept,redirect_url = auth_api:do_auth()
    return result,dept,redirect_url
end
...
-- op: string OP值
-- 例子:
-- 浏览器请求URL:http://${DOMAIN}/interface/dept/add
-- 该函数返回 add
-- #########################################################################################################
...
-- 检查用户登录态
--[[
local dept_check_result,dept_info,redirect_url = get_dept_info()
if false == dept_check_result then
    response.code = 302
...
if OP == "add" then
    ...
    if true == result then
        local business = require "dept_add"
        local result, dept_id = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = dept_id
        else
            response.data = {}
            response.data.dept_name = tbl.dept_name
            response.data.dept_id = dept_id
        end
    else
        response.code = ERR.USERINPUTFORMAT
        response.msg = errmsg
...
elseif OP == "update" then
    ...
    if true == result then
        local business = require "dept_update"
...
elseif OP == "query" then
    ...
    if true == result then
        local business = require "dept_query"
        local result,info = business:do_action(tbl.dept_id)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
        else
...
elseif OP == "delete" then
    ...
    if true == result then
        local business = require "dept_delete"
        local result,errmsg = business:do_action(tbl.dept_id)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = errmsg
        end
...
elseif OP == "query_list" then
    ...
    if true == result then
        local business = require "dept_query_list"
...
elseif OP == "tree" then
    local business = require "dept_tree"
    local result,info = business:do_action(tbl)
    if false == result then
        response.code = ERR.USERINPUTLOGICAL
        response.msg = info
