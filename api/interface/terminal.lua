...
--     content_by_lua_file ${HOME_DIR}/interface/terminal.lua;
-- }
-- 浏览器请求的地址是 http://${DOMAIN}/interface/terminal/${OP}
-- DOMAIN 服务器部署的域名
-- OP的值取决于代码中的实现
...
-- RESTFUL API定义格式见 https://github.com/changyuezhou/api_base/blob/master/doc/terminal.md
-- *********************************************************************************************************
...
function check_add_params(tbl)
    ...
    if (nil == tbl["rule_id"]) or
            ("string" ~= type(tbl["rule_id"])) then
        return false, "请检查参数rule_id.为必填且必须为字符型"
    end
    ...
    if (nil == tbl["terminal_sn"]) or
            ("string" ~= type(tbl["terminal_sn"])) then
        return false, "请检查参数terminal_sn.为必填且必须为字符型"
    end

    if (nil == tbl["air_buy_date"]) or
            ("number" ~= type(tbl["air_buy_date"])) then
        return false, "请检查参数air_buy_date.为必填且必须为整型"
    end

    if (nil ~= tbl["maintenance_records"]) and
            ("table" ~= type(tbl["maintenance_records"])) then
        return false, "请检查参数maintenance_records.为必填且必须为数组"
    end
...
function check_update_params(tbl)
    ...
    if (nil ~= tbl["etc_id"]) and
            ("string" ~= type(tbl["etc_id"])) then
        return false, "请检查参数etc_id.为必填且必须为字符型"
    end

    if (nil ~= tbl["rule_id"]) and
            ("string" ~= type(tbl["rule_id"])) then
        return false, "请检查参数rule_id.为必填且必须为字符型"
    end

    if (nil ~= tbl["air_id"]) and
            ("string" ~= type(tbl["air_id"])) then
        return false, "请检查参数air_id.为必填且必须为字符型"
    end
    ...
    if (nil ~= tbl["terminal_sn"]) and
            ("string" ~= type(tbl["terminal_sn"])) then
        return false, "请检查参数terminal_sn.为必填且必须为字符型"
    end

    if (nil ~= tbl["air_buy_date"]) and
            ("number" ~= type(tbl["air_buy_date"])) then
        return false, "请检查参数air_buy_date.为必填且必须为整型"
    end
...
function check_delete_params(tbl)
    ...
    if (nil == tbl["terminal_id"]) or
            ("table" ~= type(tbl["terminal_id"])) then
        return false, "请检查参数terminal_id.为必填且必须为字符数组"
    end
...
-- #########################################################################################################
-- 函数名: check_online_params
-- 函数功能: 校验接口 online 的参数
-- 参数定义:
-- tbl: POST请求过来的JSON数据,已经用cjson库转换成LUA TABLE
-- 返回值:
-- result: bool 表示函数成功或者失败
-- errmsg: string 表示函数失败的原因,函数校验成功时无需返回该值
-- #########################################################################################################
function check_online_params(tbl)
    if nil == tbl then
        return false, "POST数据格式错误"
    end
...
-- #########################################################################################################
-- 函数名: check_offline_params
-- 函数功能: 校验接口 offline 的参数
-- 参数定义:
-- tbl: POST请求过来的JSON数据,已经用cjson库转换成LUA TABLE
-- 返回值:
-- result: bool 表示函数成功或者失败
-- errmsg: string 表示函数失败的原因,函数校验成功时无需返回该值
-- #########################################################################################################
function check_offline_params(tbl)
    if nil == tbl then
        return false, "POST数据格式错误"
    end
...
-- #########################################################################################################
-- 函数名: check_real_status_params
-- 函数功能: 校验接口 real_status 的参数
-- 参数定义:
-- tbl: POST请求过来的JSON数据,已经用cjson库转换成LUA TABLE
-- 返回值:
-- result: bool 表示函数成功或者失败
-- errmsg: string 表示函数失败的原因,函数校验成功时无需返回该值
-- #########################################################################################################
function check_real_status_params(tbl)
    if nil == tbl then
        return false, "POST数据格式错误"
    end
...
-- #########################################################################################################
-- 函数名: check_unregister_params
-- 函数功能: 校验接口 unregister 的参数
-- 参数定义:
-- tbl: POST请求过来的JSON数据,已经用cjson库转换成LUA TABLE
-- 返回值:
-- result: bool 表示函数成功或者失败
-- errmsg: string 表示函数失败的原因,函数校验成功时无需返回该值
-- #########################################################################################################
function check_unregister_params(tbl)
    if nil == tbl then
        return false, "POST数据格式错误"
    end
...
-- #########################################################################################################
-- 函数名: check_register_params
-- 函数功能: 校验接口 register 的参数
-- 参数定义:
-- tbl: POST请求过来的JSON数据,已经用cjson库转换成LUA TABLE
-- 返回值:
-- result: bool 表示函数成功或者失败
-- errmsg: string 表示函数失败的原因,函数校验成功时无需返回该值
-- #########################################################################################################
function check_register_params(tbl)
    if nil == tbl then
        return false, "POST数据格式错误"
    end
...
-- #########################################################################################################
-- 函数名: get_terminal_info
-- 函数功能: 获取用户认证信息
-- 参数定义:
-- 无
-- 返回值:
-- result: bool 表示函数成功或者失败
-- terminal: table 用户信息对象(包含用户ID, 组ID等),失败时为nil
-- redirect_url: string 用户登录页面URL
-- #########################################################################################################
function get_terminal_info()
    local auth_api = require "authorization"
    local result,terminal,redirect_url = auth_api:do_auth()
    return result,terminal,redirect_url
end
...
-- op: string OP值
-- 例子:
-- 浏览器请求URL:http://${DOMAIN}/interface/terminal/add
-- 该函数返回 add
-- #########################################################################################################
...
-- 检查用户登录态
--[[
local terminal_check_result,terminal_info,redirect_url = get_terminal_info()
if false == terminal_check_result then
    response.code = 302
...
if OP == "add" then
    ...
    if true == result then
        local business = require "terminal_add"
        local result, terminal_id = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = terminal_id
        else
            response.data = {}
            response.data.terminal_name = tbl.terminal_name
            response.data.terminal_id = terminal_id
        end
    else
        response.code = ERR.USERINPUTFORMAT
        response.msg = errmsg
...
elseif OP == "update" then
    ...
    if true == result then
        local business = require "terminal_update"
...
elseif OP == "query" then
    ...
    if true == result then
        local business = require "terminal_query"
        local result,info = business:do_action(tbl.terminal_id)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
        else
...
elseif OP == "delete" then
    ...
    if true == result then
        local business = require "terminal_delete"
        local result,errmsg = business:do_action(tbl.terminal_id)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = errmsg
        end
...
elseif OP == "query_list" then
    ...
    if true == result then
        local business = require "terminal_query_list"
...
elseif OP == "online" then
    local result,errmsg = check_online_params(tbl)
    if true == result then
        local business = require "terminal_status_query_online_list"
        local result,info = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
...
elseif OP == "offline" then
    local result,errmsg = check_offline_params(tbl)
    if true == result then
        local business = require "terminal_status_query_offline_list"
        local result,info = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
...
elseif OP == "real_status" then
    local result,errmsg = check_real_status_params(tbl)
    if true == result then
        local business = require "terminal_status_query_real_status_list"
        local result,info = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
...
elseif OP == "unregister" then
    local result,errmsg = check_unregister_params(tbl)
    if true == result then
        local business = require "terminal_status_query_unregister_list"
        local result,info = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
...
elseif OP == "register" then
    local result,errmsg = check_register_params(tbl)
    if true == result then
        local business = require "terminal_update"
        local result,errmsg = business:do_action(tbl)
        if false == result then
...
elseif OP == "status_stat" then
    local business = require "terminal_status_stat"
    local result,info = business:do_action(tbl)
    if false == result then
        response.code = ERR.USERINPUTLOGICAL
        response.msg = info
