...
--     content_by_lua_file ${HOME_DIR}/interface/control.lua;
-- }
-- 浏览器请求的地址是 http://${DOMAIN}/interface/control/${OP}
-- DOMAIN 服务器部署的域名
-- OP的值取决于代码中的实现
...
-- RESTFUL API定义格式见 https://github.com/changyuezhou/api_base/blob/master/doc/control.md
-- *********************************************************************************************************
...
function check_add_params(tbl)
    ...
    if (nil == tbl["terminal_id"]) or
        ...
        return false, "请检查参数terminal_id.为必填且必须为数组"
    end

    if (nil == tbl["command"]) or
            ("number" ~= type(tbl["command"])) then
        return false, "请检查参数command.为必填且必须为整数"
    end
...
-- #########################################################################################################
-- 函数名: get_control_info
-- 函数功能: 获取用户认证信息
-- 参数定义:
-- 无
-- 返回值:
-- result: bool 表示函数成功或者失败
-- control: table 用户信息对象(包含用户ID, 组ID等),失败时为nil
-- redirect_url: string 用户登录页面URL
-- #########################################################################################################
function get_control_info()
    local auth_api = require "authorization"
    local result,control,redirect_url = auth_api:do_auth()
    return result,control,redirect_url
end
...
-- op: string OP值
-- 例子:
-- 浏览器请求URL:http://${DOMAIN}/interface/control/add
-- 该函数返回 add
-- #########################################################################################################
...
-- 检查用户登录态
--[[
local control_check_result,control_info,redirect_url = get_control_info()
if false == control_check_result then
    response.code = 302
...
if OP == "add" then
    ...
    if true == result then
        local business = require "control_add"
...
elseif OP == "query" then
    ...
    if true == result then
        local business = require "control_query"
...
elseif OP == "delete" then
    ...
    if true == result then
        local business = require "control_delete"
