...
--     content_by_lua_file ${HOME_DIR}/interface/alarm.lua;
-- }
-- 浏览器请求的地址是 http://${DOMAIN}/interface/alarm/${OP}
-- DOMAIN 服务器部署的域名
-- OP的值取决于代码中的实现
...
-- RESTFUL API定义格式见 https://github.com/changyuezhou/api_base/blob/master/doc/alarm.md
-- *********************************************************************************************************
...
function check_add_params(tbl)
    ...
    if (nil == tbl["alarm_name"]) or
            ("string" ~= type(tbl["alarm_name"])) then
        return false, "请检查参数alarm_name.为必填且必须为字符型"
    end
    ...
    if (nil == tbl["alarm_password"]) or
            ("string" ~= type(tbl["alarm_password"])) then
        return false, "请检查参数alarm_password.为必填且必须为字符型"
    end

    if (nil == tbl["building_info"]) or
            ("table" ~= type(tbl["building_info"])) then
        return false, "请检查参数building_info.为必填且必须为数组"
    end

    if (nil == tbl["dept_info"]) or
            ("table" ~= type(tbl["dept_info"])) then
        return false, "请检查参数dept_info.为必填且必须为数组"
    end
...
function check_update_params(tbl)
    ...
    if (nil == tbl["alarm_id"]) or
            ("string" ~= type(tbl["alarm_id"])) then
        return false, "请检查参数alarm_id.为必填且必须为字符型"
    end

    if (nil ~= tbl["alarm_name"]) and
            ("string" ~= type(tbl["alarm_name"])) then
        return false, "请检查参数alarm_name.为必填且必须为字符型"
    end
    ...
    if (nil ~= tbl["alarm_password"]) and
            ("string" ~= type(tbl["alarm_password"])) then
        return false, "请检查参数alarm_password.为必填且必须为字符型"
    end

    if (nil ~= tbl["building_info"]) and
            ("table" ~= type(tbl["building_info"])) then
        return false, "请检查参数building_info.为必填且必须为数组"
    end

    if (nil ~= tbl["dept_info"]) and
            ("table" ~= type(tbl["dept_info"])) then
        return false, "请检查参数dept_info.为必填且必须为数组"
    end
...
function check_delete_params(tbl)
    ...
    if (nil == tbl["alarm_id"]) or
            ("table" ~= type(tbl["alarm_id"])) then
        return false, "请检查参数alarm_id.为必填且必须为字符数组"
    end
...
-- 返回值:
-- result: bool 表示函数成功或者失败
-- alarm: table 用户信息对象(包含用户ID, 组ID等),失败时为nil
-- redirect_url: string 用户登录页面URL
-- #########################################################################################################
function get_alarm_info()
    local auth_api = require "authorization"
    local result,alarm,redirect_url = auth_api:do_auth()
    return result,alarm,redirect_url
end
...
-- op: string OP值
-- 例子:
-- 浏览器请求URL:http://${DOMAIN}/interface/alarm/add
-- 该函数返回 add
-- #########################################################################################################
...
-- 检查用户登录态
--[[
local alarm_check_result,alarm_info,redirect_url = get_alarm_info()
if false == alarm_check_result then
    response.code = 302
...
if OP == "query" then
    ...
    if true == result then
        local business = require "alarm_query_list"
