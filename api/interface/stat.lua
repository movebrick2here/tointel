...
--     content_by_lua_file ${HOME_DIR}/interface/stat.lua;
-- }
-- 浏览器请求的地址是 http://${DOMAIN}/interface/stat/${OP}
-- DOMAIN 服务器部署的域名
-- OP的值取决于代码中的实现
...
-- RESTFUL API定义格式见 https://github.com/changyuezhou/api_base/blob/master/doc/stat.md
-- *********************************************************************************************************
...
-- #########################################################################################################
-- 函数名: check_day_stat_params
-- 函数功能: 校验接口 day_stat 的参数
-- 参数定义:
-- tbl: POST请求过来的JSON数据,已经用cjson库转换成LUA TABLE
-- 返回值:
-- result: bool 表示函数成功或者失败
-- errmsg: string 表示函数失败的原因,函数校验成功时无需返回该值
-- #########################################################################################################
function check_day_stat_params(tbl)
    if nil == tbl then
        return false, "POST数据格式错误"
    end
    ...
    local ret,errmsg = check_query_time_params(tbl)
    if false == ret then
        return ret, errmsg
    end  
...
-- #########################################################################################################
-- 函数名: check_building_day_stat_params
-- 函数功能: 校验接口 building_day_stat 的参数
-- 参数定义:
-- tbl: POST请求过来的JSON数据,已经用cjson库转换成LUA TABLE
-- 返回值:
-- result: bool 表示函数成功或者失败
-- errmsg: string 表示函数失败的原因,函数校验成功时无需返回该值
-- #########################################################################################################
function check_building_day_stat_params(tbl)
    if nil == tbl then
        return false, "POST数据格式错误"
    end   
...
-- #########################################################################################################
-- 函数名: check_dept_day_stat_params
-- 函数功能: 校验接口 dept_day_stat 的参数
-- 参数定义:
-- tbl: POST请求过来的JSON数据,已经用cjson库转换成LUA TABLE
-- 返回值:
-- result: bool 表示函数成功或者失败
-- errmsg: string 表示函数失败的原因,函数校验成功时无需返回该值
-- #########################################################################################################
function check_dept_day_stat_params(tbl)
    if nil == tbl then
        return false, "POST数据格式错误"
    end   
...
-- #########################################################################################################
-- 函数名: check_dept_mean_consumption_params
-- 函数功能: 校验接口 dept_mean_consumption 的参数
-- 参数定义:
-- tbl: POST请求过来的JSON数据,已经用cjson库转换成LUA TABLE
-- 返回值:
-- result: bool 表示函数成功或者失败
-- errmsg: string 表示函数失败的原因,函数校验成功时无需返回该值
-- #########################################################################################################
function check_dept_mean_consumption_params(tbl)
    if nil == tbl then
        return false, "POST数据格式错误"
    end   
...
-- #########################################################################################################
-- 函数名: check_building_mean_consumption_params
-- 函数功能: 校验接口 building_mean_consumption 的参数
-- 参数定义:
-- tbl: POST请求过来的JSON数据,已经用cjson库转换成LUA TABLE
-- 返回值:
-- result: bool 表示函数成功或者失败
-- errmsg: string 表示函数失败的原因,函数校验成功时无需返回该值
-- #########################################################################################################
function check_building_mean_consumption_params(tbl)
    if nil == tbl then
        return false, "POST数据格式错误"
    end   
...
-- #########################################################################################################
-- 函数名: check_total_consumption_params
-- 函数功能: 校验接口 total_consumption 的参数
-- 参数定义:
-- tbl: POST请求过来的JSON数据,已经用cjson库转换成LUA TABLE
-- 返回值:
-- result: bool 表示函数成功或者失败
-- errmsg: string 表示函数失败的原因,函数校验成功时无需返回该值
-- #########################################################################################################
function check_total_consumption_params(tbl)
    if nil == tbl then
        return false, "POST数据格式错误"
    end
...
function check_query_params(tbl)
    ...
    if (nil == tbl["stat_id"]) or
            ("string" ~= type(tbl["stat_id"])) then
        return false, "请检查参数stat_id.为必填且必须为字符型"
    end
...
function check_delete_params(tbl)
    ...
    if (nil == tbl["stat_id"]) or
            ("table" ~= type(tbl["stat_id"])) then
        return false, "请检查参数stat_id.为必填且必须为字符数组"
    end
...
-- 返回值:
-- result: bool 表示函数成功或者失败
-- stat: table 用户信息对象(包含用户ID, 组ID等),失败时为nil
-- redirect_url: string 用户登录页面URL
-- #########################################################################################################
function get_stat_info()
    local auth_api = require "authorization"
    local result,stat,redirect_url = auth_api:do_auth()
    return result,stat,redirect_url
end
...
-- op: string OP值
-- 例子:
-- 浏览器请求URL:http://${DOMAIN}/interface/stat/add
-- 该函数返回 add
-- #########################################################################################################
...
-- 检查用户登录态
--[[
local stat_check_result,stat_info,redirect_url = get_stat_info()
if false == stat_check_result then
    response.code = 302
...
if OP == "day_stat" then
    local result,errmsg = check_day_stat_params(tbl)
    if true == result then
        local business = require "stat_query_list"
...
elseif OP == "building_day_stat" then
    local result,errmsg = check_building_day_stat_params(tbl)
    if true == result then
        local business = require "building_stat_query_list"
        local result,info = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
...
elseif OP == "dept_day_stat" then
    local result,errmsg = check_dept_day_stat_params(tbl)
    if true == result then
        local business = require "dept_stat_query_list"
        local result,info = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
...
elseif OP == "dept_mean_consumption" then
    local result,errmsg = check_dept_mean_consumption_params(tbl)
    if true == result then
        local business = require "dept_mean_consumption_query_list"
        local result,info = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
...
elseif OP == "building_mean_consumption" then
    local result,errmsg = check_building_mean_consumption_params(tbl)
    if true == result then
        local business = require "building_mean_consumption_query_list"
        local result,info = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
...
elseif OP == "total_consumption" then
    local result,errmsg = check_total_consumption_params(tbl)
    if true == result then
        local business = require "total_consumption_query"
        local result,info = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
