...
--     content_by_lua_file ${HOME_DIR}/interface/rule.lua;
-- }
-- 浏览器请求的地址是 http://${DOMAIN}/interface/rule/${OP}
-- DOMAIN 服务器部署的域名
-- OP的值取决于代码中的实现
...
-- RESTFUL API定义格式见 https://github.com/changyuezhou/api_base/blob/master/doc/rule.md
-- *********************************************************************************************************
...
function check_add_params(tbl)
    ...
    if (nil == tbl["rule_name"]) or
            ("string" ~= type(tbl["rule_name"])) then
        return false, "请检查参数rule_name.为必填且必须为字符型"
    end

    if (nil == tbl["rule_content"]) or
            ("string" ~= type(tbl["rule_content"])) then
        return false, "请检查参数rule_content.为必填且必须为字符型"
    end
...
function check_update_params(tbl)
    ...
    if (nil ~= tbl["rule_name"]) and
            ("string" ~= type(tbl["rule_name"])) then
        return false, "请检查参数rule_name.为必填且必须为字符型"
    end

    if (nil ~= tbl["rule_content"]) and
            ("string" ~= type(tbl["rule_content"])) then
        return false, "请检查参数rule_content.为必填且必须为字符型"
    end
...
function check_delete_params(tbl)
    ...
    if (nil == tbl["rule_id"]) or
            ("table" ~= type(tbl["rule_id"])) then
        return false, "请检查参数rule_id.为必填且必须为字符数组"
    end
...
-- 返回值:
-- result: bool 表示函数成功或者失败
-- rule: table 用户信息对象(包含用户ID, 组ID等),失败时为nil
-- redirect_url: string 用户登录页面URL
-- #########################################################################################################
function get_rule_info()
    local auth_api = require "authorization"
    local result,rule,redirect_url = auth_api:do_auth()
    return result,rule,redirect_url
end
...
-- op: string OP值
-- 例子:
-- 浏览器请求URL:http://${DOMAIN}/interface/rule/add
-- 该函数返回 add
-- #########################################################################################################
...
-- 检查用户登录态
--[[
local rule_check_result,rule_info,redirect_url = get_rule_info()
if false == rule_check_result then
    response.code = 302
...
if OP == "add" then
    ...
    if true == result then
        local business = require "rule_add"
        local result, rule_id = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = rule_id
        else
            response.data = {}
            response.data.rule_name = tbl.rule_name
            response.data.rule_id = rule_id
        end
    else
        response.code = ERR.USERINPUTFORMAT
        response.msg = errmsg
...
elseif OP == "update" then
    ...
    if true == result then
        local business = require "rule_update"
...
elseif OP == "query" then
    ...
    if true == result then
        local business = require "rule_query"
        local result,info = business:do_action(tbl.rule_id)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
        else
...
elseif OP == "delete" then
    ...
    if true == result then
        local business = require "rule_delete"
        local result,errmsg = business:do_action(tbl.rule_id)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = errmsg
        end
...
elseif OP == "query_list" then
    ...
    if true == result then
        local business = require "rule_query_list"
