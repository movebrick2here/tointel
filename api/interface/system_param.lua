...
--     content_by_lua_file ${HOME_DIR}/interface/system_param.lua;
-- }
-- 浏览器请求的地址是 http://${DOMAIN}/interface/system_param/${OP}
-- DOMAIN 服务器部署的域名
-- OP的值取决于代码中的实现
...
-- RESTFUL API定义格式见 https://github.com/changyuezhou/api_base/blob/master/doc/system_param.md
-- *********************************************************************************************************
...
function check_add_params(tbl)
    ...
    if (nil == tbl["platform_domain"]) or
            ("string" ~= type(tbl["platform_domain"])) then
        return false, "请检查参数platform_domain.为必填且必须为字符型"
    end

    if (nil == tbl["ip"]) or
            ("string" ~= type(tbl["ip"])) then
        return false, "请检查参数ip.为必填且必须为字符型"
    end

    if (nil == tbl["back_ip"]) or
            ("string" ~= type(tbl["back_ip"])) then
        return false, "请检查参数back_ip.为必填且必须为字符型"
    end

    if (nil == tbl["port"]) or
            ("number" ~= type(tbl["port"])) then
        return false, "请检查参数port.为必填且必须为整型"
    end

    if (nil == tbl["ssid"]) or
            ("string" ~= type(tbl["ssid"])) then
        return false, "请检查参数ssid.为必填且必须为字符型"
    end

    if (nil == tbl["ssid_password"]) or
            ("string" ~= type(tbl["ssid_password"])) then
        return false, "请检查参数ssid_password.为必填且必须为字符型"
    end

    if (nil == tbl["status_interval"]) or
            ("number" ~= type(tbl["status_interval"])) then
        return false, "请检查参数status_interval.为必填且必须为整型"
    end
...
function check_update_params(tbl)
    ...
    if (nil ~= tbl["platform_domain"]) and
            ("string" ~= type(tbl["platform_domain"])) then
        return false, "请检查参数platform_domain.为必填且必须为字符型"
    end
    ...
    if (nil ~= tbl["ssid_password"]) and
            ("string" ~= type(tbl["ssid_password"])) then
        return false, "请检查参数ssid_password.为必填且必须为字符型"
    end

    if (nil ~= tbl["status_interval"]) and
            ("number" ~= type(tbl["status_interval"])) then
        return false, "请检查参数status_interval.为必填且必须为整型"
    end
...
-- #########################################################################################################
-- 函数名: get_system_param_info
-- 函数功能: 获取用户认证信息
-- 参数定义:
-- 无
-- 返回值:
-- result: bool 表示函数成功或者失败
-- system_param: table 用户信息对象(包含用户ID, 组ID等),失败时为nil
-- redirect_url: string 用户登录页面URL
-- #########################################################################################################
function get_system_param_info()
    local auth_api = require "authorization"
    local result,system_param,redirect_url = auth_api:do_auth()
    return result,system_param,redirect_url
end
...
-- op: string OP值
-- 例子:
-- 浏览器请求URL:http://${DOMAIN}/interface/system_param/add
-- 该函数返回 add
-- #########################################################################################################
...
-- 检查用户登录态
--[[
local system_param_check_result,system_param_info,redirect_url = get_system_param_info()
if false == system_param_check_result then
    response.code = 302
...
if OP == "add" then
    ...
    if true == result then
        local business = require "system_param_add"
...
elseif OP == "update" then
    ...
    if true == result then
        local business = require "system_param_update"
        local result,errmsg = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = errmsg
...
elseif OP == "query" then
    ...
    if true == result then
        local business = require "system_param_query"
        local result,info = business:do_action()
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
        else
...
elseif OP == "delete" then
    ...
    if true == result then
        local business = require "system_param_delete"
        local result,errmsg = business:do_action()
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = errmsg
        end
