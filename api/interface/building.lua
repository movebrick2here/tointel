...
--     content_by_lua_file ${HOME_DIR}/interface/building.lua;
-- }
-- 浏览器请求的地址是 http://${DOMAIN}/interface/building/${OP}
-- DOMAIN 服务器部署的域名
-- OP的值取决于代码中的实现
...
-- RESTFUL API定义格式见 https://github.com/changyuezhou/api_base/blob/master/doc/building.md
-- *********************************************************************************************************
...
function check_add_params(tbl)
    ...
    if (nil == tbl["building_name"]) or
            ("string" ~= type(tbl["building_name"])) then
        return false, "请检查参数building_name.为必填且必须为字符型"
    end

    if (nil == tbl["parent_id"]) or
            ("string" ~= type(tbl["parent_id"])) then
        return false, "请检查参数parent_id.为必填且必须为字符型"
    end
...
function check_delete_params(tbl)
    ...
    if (nil == tbl["building_id"]) or
            ("table" ~= type(tbl["building_id"])) then
        return false, "请检查参数building_id.为必填且必须为字符数组"
    end
...
-- #########################################################################################################
-- 函数名: get_building_info
-- 函数功能: 获取用户认证信息
-- 参数定义:
-- 无
-- 返回值:
-- result: bool 表示函数成功或者失败
-- building: table 用户信息对象(包含用户ID, 组ID等),失败时为nil
-- redirect_url: string 用户登录页面URL
-- #########################################################################################################
function get_building_info()
    local auth_api = require "authorization"
    local result,building,redirect_url = auth_api:do_auth()
    return result,building,redirect_url
end
...
-- op: string OP值
-- 例子:
-- 浏览器请求URL:http://${DOMAIN}/interface/building/add
-- 该函数返回 add
-- #########################################################################################################
...
-- 检查用户登录态
--[[
local building_check_result,building_info,redirect_url = get_building_info()
if false == building_check_result then
    response.code = 302
...
if OP == "add" then
    ...
    if true == result then
        local business = require "building_add"
        local result, building_id = business:do_action(tbl)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = building_id
        else
            response.data = {}
            response.data.building_name = tbl.building_name
...
elseif OP == "update" then
    ...
    if true == result then
        local business = require "building_update"
...
elseif OP == "query" then
    ...
    if true == result then
        local business = require "building_query"
        local result,info = business:do_action(tbl.building_id)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = info
        else
...
elseif OP == "delete" then
    ...
    if true == result then
        local business = require "building_delete"
        local result,errmsg = business:do_action(tbl.building_id)
        if false == result then
            response.code = ERR.USERINPUTLOGICAL
            response.msg = errmsg
        end
...
elseif OP == "query_list" then
    ...
    if true == result then
        local business = require "building_query_list"
...
elseif OP == "tree" then
    local business = require "building_tree"
    local result,info = business:do_action(tbl)
    if false == result then
        response.code = ERR.USERINPUTLOGICAL
        response.msg = info
